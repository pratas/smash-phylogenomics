...
//  lambda = (1 + lambda*lambda*lambda + tanh(8*(lambda-1))) / 2; 
  ...
  double phi = 2*M_PI*(start/3+rotations*lambda);
  double lambdaGamma = pow(lambda, gamma);
  double a = hue*lambdaGamma*(1-lambdaGamma)/2;
  ...
  sprintf(color, "#%02X%02X%02X", (int) (R * 255), (int) (G * 255), (int) (B * 255));
  return color;
  }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

Painter *CreatePainter(double size)
  {
